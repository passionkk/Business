...
{
    FlvMuxer *pMuxer = (FlvMuxer *)pTSMuxer;
...
void FlvMuxer::Subscribe(DataHandle dataHandle, void *pUserData)
{
...
void FlvMuxer::UnSubscribe(DataHandle dataHandle)
{
    Poco::Mutex::ScopedLock lock(m_CallbackMutex);
...
void FlvMuxer::Subscribe(FlvDataIface *pDataInstance, void *pUserData)
{
    Poco::Mutex::ScopedLock lock(m_CallbackMutex);
    bool bNotFind = true;
...
void FlvMuxer::UnSubscribe(FlvDataIface *pDataInstance)
{
    Poco::Mutex::ScopedLock lock(m_CallbackMutex);
	for (std::vector<std::pair<FlvDataIface *, void*> >::iterator it =
...
bool FlvMuxer::IsRunning() const
{
    return m_RoutineThread.isRunning();
}

int FlvMuxer::ReadAVData(uint8_t *pBuf, int iBufSize, MediaType eType)
{
    int iReadSize = 0;
    do
    {
...
int FlvMuxer::OnDataHandle(uint8_t *pBuf, int iBufSize)
{
    Poco::Mutex::ScopedLock lock(m_CallbackMutex);
    for (int i = 0; i < m_DataHandles.size(); ++i)
    {
...
AVInputFormat *FlvMuxer::GetAVInputFormat(FormatType eType)
{
...
void FlvMuxer::SetAVPacketFlag(AVPacket *pkt, MediaType eType)
{
    if (eType == Audio)
    {
